...
// Confetti layout is randomised once per app launch (module scope), not per render.
...
export default function LevelUp() {
  ...
  const rewardLevel = levelRewards.some((r) => r.level === level) ? level : Math.max(0, ...levelRewards.filter((r) => r.level <= level).map((r) => r.level));
  const current = levelRewards.filter((r) => r.level === rewardLevel).slice(0, 3);
  ...
  return (
    <View style={styles.root}>
      ...
      <Confetti visible={isLevelUp && showConfetti} />
