...
function Sneaker({ x, y, dir, rot, color }: { x: number; y: number; dir: number; rot: number; color: string }) {
  ...
  return (
    <G transform={rot ? `rotate(${rot} ${x} ${y})` : undefined}>
      ...
      <Rect x={f(x - 10.5 + o)} y={y - 5.5} width={21} height={5.5} rx={2.4} fill="#F4F4EF" />
      <Path d={`M${f(x - 10 + o)},${y - 2.2} L${f(x + 10.2 + o)},${y - 2.2}`} stroke="#CED0C3" strokeWidth={0.9} />
...
function Dumbbell({ p }: { p: Pt }) {
  ...
  return (
    <G>
      ...
      <Rect x={x - 13} y={y - 6} width={5} height={12} rx={1.6} fill="#2C2E1B" />
      <Rect x={x + 8} y={y - 6} width={5} height={12} rx={1.6} fill="#2C2E1B" />
...
function TopGarment({ look, g, uid }: TopProps) {
  ...
  switch (look.top) {
    ...
    default:
      ...
      extras = (
        <>
          <Path d={`M56,${NECK_Y + 2} L64,${NECK_Y + 2} L65.5,134 L54.5,134Z`} fill="#F4F5EF" />
...
function Face({ look, expr, fx, skinDark, uid }: { look: AvatarLook; expr: Expr; fx: number; skinDark: string; uid: string }) {
  ...
  const lip = female ? '#AD793F' : '#874733';
  ...
  return (
    <G>
      ...
      <G transform={fx ? `translate(${fx},0)` : undefined}>
        ...
        {expr === 'grin' ? (
          <>
            <Path d="M55.6,44.6 Q60,44 64.4,44.6 Q62.6,49.4 60,49.4 Q57.4,49.4 55.6,44.6Z" fill="#573A19" />
            <Path d="M56.4,45 Q60,44.5 63.6,45 L63.1,46.2 Q60,45.8 56.9,46.2Z" fill={art.white} />
          </>
        ) : expr === 'open' ? (
          <Ellipse cx={60} cy={46.4} rx={2.2} ry={2} fill="#573A19" />
        ) : (
...
function Accessory({ look, fx }: { look: AvatarLook; fx: number }) {
  switch (look.accessory) {
    case 'shades':
      return (
        <G transform={fx ? `translate(${fx},0)` : undefined}>
          ...
          <Path d="M48.6,32 L58,32 Q58.4,32 58.3,32.6 L57.6,37 Q57.2,39 55,39 L51.6,39 Q49.4,39 49,37 L48.3,32.6 Q48.3,32 48.6,32Z" fill="#191A10" stroke={art.pink} strokeWidth={1.1} />
          <Path d="M71.4,32 L62,32 Q61.6,32 61.7,32.6 L62.4,37 Q62.8,39 65,39 L68.4,39 Q70.6,39 71,37 L71.7,32.6 Q71.7,32 71.4,32Z" fill="#191A10" stroke={art.pink} strokeWidth={1.1} />
    ...
    case 'cap': {
      const capC = luma(look.topColor) > 0.85 ? '#1A1A13' : look.topColor;
    ...
    case 'headphones':
      return (
        <G>
          <Path d="M44.5,33 C42,4 78,4 75.5,33" stroke="#1A1A13" strokeWidth={2.8} fill="none" strokeLinecap="round" />
          <Path d="M47,20 C49,13 54,10 59,9.5" stroke="#414334" strokeWidth={1} fill="none" strokeLinecap="round" />
...
export const demoLooks: AvatarLook[] = [
  { body: 'female', skin: skinTones[1], hair: 'ponytail', hairColor: hairColors[5], top: 'crop', topColor: '#1A1A13', bottom: 'leggings', bottomColor: '#1A1A13', shoeColor: art.pink, accessory: 'shades' },
  { body: 'male', skin: skinTones[4], hair: 'short', hairColor: hairColors[0], top: 'hoodie', topColor: art.purple, bottom: 'joggers', bottomColor: '#222319', shoeColor: art.cyan, accessory: 'headphones' },
  { body: 'female', skin: skinTones[5], hair: 'afro', hairColor: hairColors[0], top: 'tank', topColor: art.cyan, bottom: 'shorts', bottomColor: '#1A1A13', shoeColor: art.white, accessory: 'headband' },
  { body: 'male', skin: skinTones[0], hair: 'buzz', hairColor: hairColors[4], top: 'tee', topColor: art.white, bottom: 'shorts', bottomColor: art.pink, shoeColor: '#1A1A13', accessory: 'cap' },
  { body: 'female', skin: skinTones[0], hair: 'long', hairColor: hairColors[4], top: 'jacket', topColor: art.pink, bottom: 'leggings', bottomColor: '#1A1A13', shoeColor: art.white, accessory: 'none' },
  { body: 'male', skin: skinTones[3], hair: 'curly', hairColor: hairColors[1], top: 'tank', topColor: art.orange, bottom: 'joggers', bottomColor: '#2F3126', shoeColor: art.yellow, accessory: 'none' },
  ...
  { body: 'male', skin: skinTones[5], hair: 'afro', hairColor: hairColors[6], top: 'jacket', topColor: '#1A1A13', bottom: 'joggers', bottomColor: '#1A1A13', shoeColor: art.pink, accessory: 'shades' },
  { body: 'female', skin: skinTones[3], hair: 'bob', hairColor: hairColors[0], top: 'hoodie', topColor: art.pink, bottom: 'joggers', bottomColor: '#3B3F1F', shoeColor: art.white, accessory: 'none' },
  { body: 'male', skin: skinTones[2], hair: 'short', hairColor: hairColors[3], top: 'crop', topColor: art.green, bottom: 'shorts', bottomColor: '#1A1A13', shoeColor: art.orange, accessory: 'cap' },
];
