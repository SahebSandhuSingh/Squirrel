...
function Confetti({ visible }: { visible: boolean }) {
  const pieces = useRef(
    Array.from({ length: 30 }, () => ({
      ...
      color: ['#E3CB8F', '#E9B949', '#9DB04C', '#B4C27A', '#E07A5F', '#9CCB7A'][Math.floor(Math.random() * 6)],
...
const styles = StyleSheet.create({
  ...
  gain: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(14,16,11,0.7)', borderRadius: radius.pill, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: colors.line },
  gainText: { color: colors.primary, fontFamily: fonts.bold, fontSize: 13 },
  next: { flexDirection: 'row', alignItems: 'center', marginVertical: 14, backgroundColor: 'rgba(26,29,20,0.94)', borderRadius: radius.lg, borderWidth: 1, borderColor: 'rgba(157,176,76,0.5)', padding: 14 },
