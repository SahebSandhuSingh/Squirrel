...
export const productDefaults: Record<ProductKind, { color: string; accent: string }> = {
  ...
  jacket: { color: '#3E3E40', accent: art.pink },
  joggers: { color: '#48474A', accent: art.pink },
  ...
  gloves: { color: '#605F64', accent: art.pink },
  mat: { color: art.green, accent: art.purple },
  dumbbell: { color: '#2B2A2C', accent: art.pink },
  kettlebell: { color: '#171717', accent: art.cyan },
...
const draw: Record<ProductKind, (x: Ctx) => React.ReactElement> = {
  ...
  bottle: ({ uid, c, a, k }) => (
    <G>
      ...
      <Path d="M52 45 H68 C70 45 71 46 71 48 V96 C71 98 70 99 68 99 H52 C50 99 49 98 49 96 V48 C49 46 50 45 52 45 Z" fill="#201F21" opacity={0.85} />
      <G clipPath={`url(#${uid}ow)`}>
        <Rect x={48} y={62} width={24} height={40} fill={`url(#${uid}of)`} />
        <Rect x={48} y={62} width={24} height={2.5} fill="#D9DDDE" />
        <Path d={`${circ(56, 78, 1.6)} ${circ(63, 86, 1.2)} ${circ(58, 91, 1)}`} fill="#EEF0F0" opacity={0.8} />
  ...
  watch: ({ uid, c, a, k }) => (
    <G>
      ...
      <Path d="M42 40 C42 36 44 33 48 33 H72 C76 33 78 36 78 40 V80 C78 84 76 87 72 87 H48 C44 87 42 84 42 80 Z" fill="#090909" />
...
export const ProductArt = React.memo(function ProductArt(props: {
  ...
  const ctx = React.useMemo<Ctx>(() => {
    ...
    return {
      ...
      w: makeTone('#F6F5F6', rim, `${uid}gw`),
      k: makeTone(mix(color, '#131214', 0.72), rim, `${uid}gk`),
      rib: makeTone(mix(color, '#131214', 0.3), rim, `${uid}gr`),
    };
  }, [uid, color, accent]);
