...
import { Avatar3DViewer } from '@/art/Avatar3DViewer';
import { Mascot } from '@/art/Mascot';
import { ProductArt } from '@/art/Product';
import { ShopItemCard } from '@/components/cards';
import { Button, FadeIn, Icon, IconButton, Label, Segmented, Tagline, tap } from '@/components/ui';
import { avatar3DModels } from '@/data/avatar3DModels';
import {
  accessoryExtras,
  accessoryStyles,
  avatarCategories,
...
export default function AvatarScreen() {
  ...
  const [model3D, setModel3D] = useState(avatar3DModels[0].id);
  // The 3D preview runs in a WebView, which react-native-web doesn't provide.
  const canUse3D = Platform.OS !== 'web' && avatar3DModels.length > 0;
  const is3D = canUse3D && viewMode === '3D';
  const selectedModel = avatar3DModels.find((m) => m.id === model3D) ?? avatar3DModels[0];
  ...
  return (
    <View style={styles.root}>
      <LinearGradient colors={['#E9EDF7', '#F4F6FB', '#FFFFFF']} style={StyleSheet.absoluteFill} />
      <View style={[styles.col, { paddingTop: insets.top + 6 }]}>
        ...
        {canUse3D && (
          <View style={styles.modeRow}>
            <Segmented items={['2D', '3D'] as const} value={viewMode} onChange={setViewMode} />
          </View>
        )}
        ...
        {is3D ? (
          <View style={[styles.stage, { height: stageH }]}>
            <ScrollView style={styles.presetCol} contentContainerStyle={{ gap: 10, paddingVertical: 6 }} showsVerticalScrollIndicator={false}>
              {avatar3DModels.map((m) => {
                const on = m.id === model3D;
                return (
                  <Pressable key={m.id} onPress={() => { tap(); setModel3D(m.id); }} style={[styles.model3DChip, on && styles.model3DChipOn]} accessibilityLabel={m.name}>
                    <Text style={[styles.model3DChipText, on && { color: colors.primary }]} numberOfLines={2}>{m.name}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              <FadeIn key={model3D} from={6}>
                <Avatar3DViewer modelFile={selectedModel.file} size={stageH * 0.86} />
              </FadeIn>
            </View>
          </View>
        ) : (
        ...
        {is3D ? (
          <View style={styles.panel}>
            <Label style={{ marginBottom: 6 }}>3D Preview — prototype scan</Label>
            <Text style={styles.scanNote}>
              Raw AI-generated scan, not yet game-ready (high-poly, uncompressed). Decimate before shipping — see README.
              {selectedModel.hasThirdPartyBranding ? ' This scan also has real third-party branding baked into its texture — swap or re-texture before any public build.' : ''}
            </Text>
          </View>
        ) : (
        <View style={styles.panel}>
...
const styles = StyleSheet.create({
  ...
  modeRow: { paddingHorizontal: 12, marginTop: 10, alignItems: 'center' },
  stage: { flexDirection: 'row', paddingHorizontal: 12, marginTop: 8 },
  presetCol: { flexGrow: 0, width: 58 },
  model3DChip: { minHeight: 50, borderRadius: radius.md, borderWidth: 1.5, borderColor: colors.line, backgroundColor: colors.glass, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4, paddingVertical: 6 },
  model3DChipOn: { borderColor: colors.primary, backgroundColor: 'rgba(47,91,255,0.12)' },
  model3DChipText: { color: colors.sub, fontFamily: fonts.semibold, fontSize: 10, textAlign: 'center' },
  scanNote: { color: colors.sub, fontFamily: fonts.medium, fontSize: 12, lineHeight: 17, paddingRight: 16 },
  ...
  platform: { position: 'absolute', bottom: 0, width: 170, height: 26, borderRadius: 85, backgroundColor: 'rgba(47,91,255,0.18)', borderWidth: 1.5, borderColor: 'rgba(47,91,255,0.5)' },
  ...
  railOn: { borderColor: colors.primary, backgroundColor: 'rgba(47,91,255,0.12)' },
  railText: { color: colors.sub, fontSize: 10, fontFamily: fonts.semibold },
  panel: { marginTop: 14, paddingBottom: 14, paddingLeft: 16, paddingTop: 12, borderTopWidth: 1, borderTopColor: colors.line, backgroundColor: colors.bg2 },
  ...
  optOn: { borderColor: colors.primary, backgroundColor: 'rgba(47,91,255,0.1)' },
