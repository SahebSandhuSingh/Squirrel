...
export default function Home() {
  const { me, missions, logMission, claimed, claimable, claimRewards, events, joinedEvents, toggleEvent, city, districts, unfinishedRun, pendingUploads } = useApp();
  ...
  return (
    <Screen>
      ...
      {/* Runs that need attention: an unfinished run (app was killed mid-run) or uploads that failed. */}
      {unfinishedRun && (
        <FadeIn index={2}>
          <Pressable onPress={() => router.push({ pathname: '/run', params: { resume: '1' } })} style={styles.banner} accessibilityRole="button">
            <Icon name="run-fast" size={20} color={colors.primary} />
            <View style={{ flex: 1 }}>
              <Text style={styles.bannerTitle}>You have an unfinished run</Text>
              <Text style={styles.bannerSub}>Tap to pick up where you left off, or finish and claim it.</Text>
            </View>
            <Icon name="chevron-right" size={18} color={colors.primary} />
          </Pressable>
        </FadeIn>
      )}
      {!unfinishedRun && pendingUploads > 0 && (
        <FadeIn index={2}>
          <Pressable onPress={() => router.push({ pathname: '/run', params: { retry: '1' } })} style={styles.banner} accessibilityRole="button">
            <Icon name="cloud-upload-outline" size={20} color={colors.primary} />
            <View style={{ flex: 1 }}>
              <Text style={styles.bannerTitle}>{pendingUploads} run{pendingUploads > 1 ? 's' : ''} waiting to upload</Text>
              <Text style={styles.bannerSub}>Saved on this phone. Tap to retry — nothing is lost.</Text>
            </View>
            <Icon name="chevron-right" size={18} color={colors.primary} />
          </Pressable>
        </FadeIn>
...
const styles = StyleSheet.create({
  banner: { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 14, backgroundColor: 'rgba(47,91,255,0.08)', borderRadius: radius.md, borderWidth: 1, borderColor: 'rgba(47,91,255,0.4)', padding: 12 },
  bannerTitle: { color: colors.text, fontFamily: fonts.semibold, fontSize: 13 },
  bannerSub: { color: colors.dim, fontFamily: fonts.regular, fontSize: 12, marginTop: 2 },
